...
// commented out [id="assembly-adding-sources-creds-gui-main-{context}"]

[id="assembly-adding-sources-creds-gui-main"]

= Adding sources and credentials

To prepare {ProductNameShort} to run scans, you must add the parts of your IT infrastructure that you want to scan as one or more sources. You must also add the authentication information, such as a username and password or SSH key, that is required to access those sources as one or more credentials. You add sources and credentials according to the type of source that you are going to scan.
...
* xref:assembly-adding-net-sources-creds-gui-{context}[Adding network sources and credentials]
...
* xref:assembly-adding-sat-sources-creds-gui-{context}[Adding satellite sources and credentials]
...
* xref:assembly-adding-vcent-sources-creds-gui-{context}[Adding vcenter sources and credentials]
